...
impl Abomonation for HostId{}
impl <> record::FromRecord for HostId<> {
    fn from_record(val: &record::Record) -> Result<Self, String> {
        match val {
            record::Record::PosStruct(constr, args) => {
                match constr.as_ref() {
                    "HostId" if args.len() == 1 => {
                        Ok(HostId{id: <types_UUID>::from_record(&args[0])?})
                    },
                    c => Result::Err(format!("unknown constructor {} of type HostId in {:?}", c, *val))
                }
            },
            record::Record::NamedStruct(constr, args) => {
                match constr.as_ref() {
                    "HostId" => {
                        Ok(HostId{id: record::arg_extract::<types_UUID>(args, "id")?})
                    },
                    c => Result::Err(format!("unknown constructor {} of type HostId in {:?}", c, *val))
                }
            },
...
decl_struct_into_record!(HostId, <>, id);
decl_record_mutator_struct!(HostId, <>, id: types_UUID);
impl fmt::Display for HostId {
    fn fmt(&self, __formatter: &mut fmt::Formatter) -> fmt::Result {
        match self {
            HostId{id} => {
                __formatter.write_str("HostId{")?;
...
impl Abomonation for VM_{}
impl <> record::FromRecord for VM_<> {
    fn from_record(val: &record::Record) -> Result<Self, String> {
        match val {
            record::Record::PosStruct(constr, args) => {
                match constr.as_ref() {
                    "VM_" if args.len() == 1 => {
                        Ok(VM_{vip: <types_VIP>::from_record(&args[0])?})
                    },
                    c => Result::Err(format!("unknown constructor {} of type VM_ in {:?}", c, *val))
                }
            },
            record::Record::NamedStruct(constr, args) => {
                match constr.as_ref() {
                    "VM_" => {
                        Ok(VM_{vip: record::arg_extract::<types_VIP>(args, "vip")?})
                    },
                    c => Result::Err(format!("unknown constructor {} of type VM_ in {:?}", c, *val))
                }
            },
...
decl_struct_into_record!(VM_, <>, vip);
decl_record_mutator_struct!(VM_, <>, vip: types_VIP);
impl fmt::Display for VM_ {
    fn fmt(&self, __formatter: &mut fmt::Formatter) -> fmt::Result {
        match self {
            VM_{vip} => {
                __formatter.write_str("VM_{")?;
                fmt::Debug::fmt(vip, __formatter)?;
                __formatter.write_str("}")
            }
        }
...
pub fn relname2id(rname: &str) -> Option<Relations> {
   match rname {
        ...
        "HostId" => Some(Relations::HostId),
        "VM" => Some(Relations::VM),
        "VM_" => Some(Relations::VM_),
       _  => None
   }
}
...
pub fn relid2rel(rid: RelId) -> Option<Relations> {
   match rid {
        ...
        1 => Some(Relations::HostId),
        2 => Some(Relations::VM),
        3 => Some(Relations::VM_),
       _  => None
   }
}
...
lazy_static! {
    pub static ref RELIDMAP: FnvHashMap<Relations, &'static str> = {
        ...
        m.insert(Relations::HostId, "HostId");
        m.insert(Relations::VM, "VM");
        m.insert(Relations::VM_, "VM_");
        m
   };
}
lazy_static! {
...
pub fn relval_from_record(rel: Relations, rec: &record::Record) -> Result<Value, String> {
    match rel {
        ...
        Relations::HostId => {
            Ok(Value::HostId(<HostId>::from_record(rec)?))
        },
        Relations::VM => {
            Ok(Value::VM(boxed::Box::new(<VM>::from_record(rec)?)))
        },
        Relations::VM_ => {
            Ok(Value::VM_(<VM_>::from_record(rec)?))
        }
    }
}
...
impl fmt::Display for Value {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        match self {
            ...
            Value::HostId (v) => write!(f, "{:?}", *v),
            Value::VM (v) => write!(f, "{:?}", *v),
            Value::VM_ (v) => write!(f, "{:?}", *v)
        }
    }
}
decl_val_enum_into_record!(Value, <>, tuple0__(x), FwdTable(x), HostId(x), VM(x), VM_(x));
decl_record_mutator_val_enum!(Value, <>, tuple0__(()), FwdTable(FwdTable), HostId(HostId), VM(VM), VM_(VM_));
...
pub fn prog(__update_cb: Box<dyn CBFn<Value>>) -> Program<Value> {
    ...
    let HostId = Relation {
                     name:         "HostId".to_string(),
                     input:        true,
                     distinct:     false,
                     key_func:     None,
                     id:           Relations::HostId as RelId,
                     rules:        vec![
                         ],
                     arrangements: vec![
                         Arrangement::Map{
    ...
    let VM = Relation {
                 ...
                 rules:        vec![
                     /* VM(.vip=vip, .host=host) :- VM_(.vip=vip), HostId(.id=host). */
                     Rule::ArrangementRule {
                         description: "VM(.vip=vip, .host=host) :- VM_(.vip=vip), HostId(.id=host).".to_string(),
                         arr: ( Relations::VM_ as RelId, 0),
                         xform: XFormArrangement::Join{
                                    description: "VM_(.vip=vip), HostId(.id=host)".to_string(),
                                    ffun: None,
                                    arrangement: (Relations::HostId as RelId,0),
                                    jfun: &{fn __f(_: &Value ,__v1: &Value,__v2: &Value) -> Option<Value>
                                    {
                                        ...
                                        let host = match *__v2{
                                            Value::HostId(ref __box) => {
                                                match *__box {
                                                HostId{id: ref host} => host,
                                                _ => return None
                                                }
                                            },
                                            _ => return None
                                        };
                                        Some(Value::VM(boxed::Box::new(VM{vip: vip.clone(), host: host.clone()})))
                                    }
                                    __f},
                                    next: Box::new(None)
                                }
    ...
    Program {
        nodes: vec![
            ...
            ProgNode::Rel{rel: HostId},
            ProgNode::Rel{rel: VM_},
            ProgNode::Rel{rel: VM}
        ],
        init_data: vec![
        ]
